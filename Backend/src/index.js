...
// ============================================================================
// X) PREPRODUCCIÓN (preproduccion_valores)
// ============================================================================

// GET /preproduccion-valores  -> lista
app.get('/preproduccion-valores', async (_req, res) => {
  try {
    const { rows } = await pool.query(
      `
...
// Body: { nv?: int|null, patch?: object }
// - nv: opcional
// - patch: mergea sobre data jsonb (data || patch)
app.put('/preproduccion-valores/:id', async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id)) return res.status(400).json({ error: 'id inválido' });
  ...
  // nv opcional (si viene, debe ser int o null)
  let nv;
  ...
  // patch opcional (si viene, debe ser objeto)
  let patchObj = {};
  ...
  // Si no hay nada para actualizar, evitamos update vacío
  if (!hasNv && !hasPatch) {
